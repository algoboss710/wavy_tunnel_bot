...
async function fetchAllUsers(ip: string, baseUrl: string): Promise<any[]> {
    ...
    try {
        // Make a fetch request to the specified URL and await the response
        const response = await fetch(nextPageUrl);
        // Parse the JSON response and store the user data in the allUsers array
...
/*
  Outline for Setting Up Rate-Limited Data Fetching and Processing
  ...
  3. Fetch Data from an API:
     - Write an asynchronous function to retrieve data from a specified API endpoint.
     - Ensure the rate limiter is called before making the API request to enforce the constraints.
     - Handle potential errors that may occur during the data fetching process.
  ...
  6. Adapt and Reuse:
     ...
     - Use this outline as a template for similar problems involving rate-limited API requests and data processing.
*/
